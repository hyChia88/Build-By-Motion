...
'''
Structure take reference from Week7 assignment tetris3D
'''
class Grid3D:
...
def removeCell(app, cell):
    if (0 <= app.currentX < app.gridSize and 
        0 <= app.currentY < app.gridSize and 
        0 <= app.currentZ < app.gridSize):
        if cell is not None and cell.getPattern() is not None:
            print("cell @ removeCell:")
            print(cell)
            print("cell.getPattern() @ removeCell:")
            print(cell.getPattern())
...
class LShapeCell(Cell):
    def __init__(self, x, y, z, fracLevel, app):
        ...
        # Use False instead of None, so that at getPlacementPos, it will be gently ignored
        self.name = "LShape"
...
class ImageCell(Cell):
    def __init__(self, x, y, z, fracLevel, app):
        ...
        if app.image is not None:
            ...
            imagePath = app.image
            result = self.processImg(imagePath)
            newArr = self.reMap(app.gridSize, app.gridSize, result)
    ...
    '''
    convert an RGB image to Boolean array method, debugging with AI
    https://answers.opencv.org/question/216848/how-to-convert-an-rgb-image-to-boolean-array/
    https://stackoverflow.com/questions/70574748/what-settings-in-cv2-threshold-to-threshold-this-image
    ...
    def processImg(self,filename):
        ...
        '''
        Line226 is generated by GenAI, asking how to convert a numpy array to boolean array, and it suggested this line
        '''
        boolArr = img < 20
...
'''
Mainly reference from https://youtu.be/RRBXVu5UE-U?si=FTBWxNPHmmu-KmW6 and modified to fit my needs
'''
class HandGestureDetector:
    ...
    def detectGesture(self):
        ...
        cv2.waitKey(1)
        # Make sure the value is between 0 and 1, if not, set it to 0 or 1 avoid it out of range (0,1)
...
Asked ChatGPT to recommend with the formula, and it recommended the formula below:
               | cos(Y)  0  sin(Y) | | 1   0   0  |          | cos(Y) sin(Y)sin(X) sin(Y)cos(X) |
R = R(Y)R(X) = | 0      1   0      | | 0  cos(X) -sin(X) | = | 0         cos(X)        -sin(X)  |
               | -sin(Y) 0  cos(Y) | | 0  sin(X)  cos(X) |   | -sin(Y) cos(Y)sin(X) cos(Y)cos(X)|

To avoid using numpy, I break it down to math formula instead of directly using matrix multiplication
Debugging with AI.
'''
class Projection3D:
    def __init__(self):
    ...
    def basicProj(self, app, x, y, z, rotationY,rotationX):
        ...
        screenX = app.boardLeft + app.boardWidth*(2/3) + finalX * app.cellSize * app.scale
...
class Draw:
    ...
    '''
    Below are the methods for manipulating cells
    ...
    def getCellPoints(self, posList):
        # Template cube points and faces, which is hard code
        CUBE_POINTS = [
    ...
    # Clean mesh by removing duplicate points and faces. Got cleanPts and cleanFaces from getCellPoints
    def cleanMesh(self, posList):
        ...
        for point in allPts:
            ...
            if pointTuple not in pointMap:
                pointMap[pointTuple] = len(cleanedPts)
                cleanedPts.append(point)
            old_to_new_idx[i] = pointMap[pointTuple]
            i += 1
    ...
    from line 491 to 788, from getEdgeFace to getOutput in this file are mostly copied and also modified to this python file
    '''
    ...
    '''
    F: face center; R: edge center; P: vertex point; n: number of faces barycenter of P, R and F with respective weights (n − 3), 2 and 1
    '''
    def get_newPoints(self, input_points, points_faces, avg_face_points, avg_mid_edges):
        newPoints =[]
    ...
    def cmc_subdiv(self, input_points, input_faces):
        ...
        new_coords = (m1 * old_coords)
                ...
                + (m3 * avg_mid_edges)
        """
        newPoints = self.get_newPoints(input_points, points_faces, avg_face_points, avg_mid_edges)
        
        face_point_nums = []
        
        next_pointnum = len(newPoints)
        
        for face_point in face_points:
            newPoints.append(face_point)
...
def drawCell(app, posList, isConfirmed):
    ...
    for face in allfaces:
        ...
        if isConfirmed:
            drawPolygon(pt1[0], pt1[1], pt2[0], pt2[1], pt3[0], pt3[1], pt4[0], pt4[1],pt1[0], pt1[1], 
                        fill = "white", border ='grey',borderWidth =1,opacity=40)
        else:
            drawPolygon(pt1[0], pt1[1], pt2[0], pt2[1], pt3[0], pt3[1], pt4[0], pt4[1],pt1[0], pt1[1], 
                        fill = "white", border ="blue" if app.grid.isPosValid(app.cell) else 'red',borderWidth =1,opacity=40)
...
def build_onMousePress(app, mouseX, mouseY):
    ...
    markingY = app.instructionY + app.spacing*18 + app.frameImgSize
    ...
    # Draw button
    axisX = app.frameImgX + app.frameImgSize/2 +10
...
def build_onKeyPress(app, key):
    if key == 'space':
        ...
        print("app.cell @ removeCell onkeypress:")
        print(app.cell)
        removeCell(app, app.cell)
...
def build_redrawAll(app):
    ...
    drawLabel('• Use hand gestures or W/A/S/D to move the cube in X/Y plane', axisX, app.instructionY + app.spacing,size=app.normalFS)
    drawLabel('• Hold index and middle fingers together or use Q/E to move in Z axis', axisX, app.instructionY + app.spacing*2,size=app.normalFS)
    ...
    drawLabel('• 7: Change to ImageCell', axisX, app.instructionY + app.spacing*8,size=app.normalFS)

    # Special features
    drawLabel('Special Features:', axisX, app.instructionY + app.spacing*10, size=app.normalFS, bold=True)
    drawLabel('• X: delete current cell', axisX, app.instructionY + app.spacing*11,size=app.normalFS)
    drawLabel('• [ ]: Adjust subdivide level (current: ' + str(app.subdLvl) + ')', axisX, app.instructionY + app.spacing*12,size=app.normalFS)
    drawLabel('• R: Reset game; right/left: Change grid size; up/down: Change zoom in/out', axisX, app.instructionY + app.spacing*13,size=app.normalFS)
    ...
    markingY = app.instructionY + app.spacing*18 + app.frameImgSize
    # import the image 
    if app.importPattern:
        drawLabel("Drawn Pattern exists, press 7 to place", app.frameImgX+app.frameImgSize/2, app.instructionY + app.spacing*16 + app.frameImgSize/2, size=app.normalFS, align = "center")
    elif app.image is None: # or not app.loadImageFromStringReference(app.image)
        drawLabel("No image file! Import imageCell.jpg", app.frameImgX+app.frameImgSize/2, markingY-8, size=app.normalFS, align = "center")

    if app.image == "imageCell.jpg" and app.showImage:
        try:
            drawImage("imageCell.jpg", app.frameImgX, app.instructionY + app.spacing*16, width=app.frameImgSize, height=app.frameImgSize)
            drawLabel("imageCell.jpg exists, press 7 to place", app.frameImgX+app.frameImgSize/2, markingY-8, size=app.normalFS, align = "center")
        except:
            pass
